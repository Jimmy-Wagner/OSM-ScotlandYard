...
public class ReducedWay implements Comparable, Cloneable{
    ...
    // The Integer saves the position in the waypoints list
    ...
    public void initializeKdTree() {
        // Build kd tree with dimension 2
        this.kdTree = new KDTree<Integer>(2);
        int indexOfPoint = 0;
        for (Point point : this.wayPoints) {
            try {
                kdTree.insert(new double[]{point.getX(), point.getY()}, indexOfPoint++);
    ...
    public int findNearestCoordinateIndex(Coordinate coordinate) {
        ...
        try {
            nearestPointIndex = this.kdTree.nearest(coordinateToDouble(coordinate));
        } catch (KeySizeException e) {
            e.printStackTrace();
        }
